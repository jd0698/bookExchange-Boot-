...
<%@page import="com.masterBook.model.BookDataVO"%>
...
<body>
	...
	<c:choose>
		<c:when test="${empty requestScope.listOfAllBooksByUser}">
			No Books!!	
		</c:when>
		<c:otherwise>
			<table>
			...
			<c:forEach items="${requestScope.listOfAllBooksByUser}" var="book">
				<tr>
				    <td>${book.title}</td>
				    <td>${book.pubYear}</td>
				    <td>
